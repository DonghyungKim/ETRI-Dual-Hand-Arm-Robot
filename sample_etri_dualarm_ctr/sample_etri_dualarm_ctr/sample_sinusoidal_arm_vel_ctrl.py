...
class JointCommandPublisher(Node):
    def __init__(self):
        ...
        self.default_velocity = np.zeros(49, dtype=float)

        self.joint_command.velocity = [float(x) for x in self.default_velocity]
    ...
    def publish_sinusoidal_joint_velocity(self, joint_names, amplitude):
        """Publish sine wave velocities for the specified joints."""
        total_duration = ITERATIONS / SINE_FREQUENCY
        start_time = time.time()
        ...
        # default_position = self.default_position[joint_indices]
        default_velocity = self.default_velocity[joint_indices]

        while time.time() - start_time < total_duration:
            elapsed_time = time.time() - start_time

            if (time.time() - start_time) >= (total_duration - 1 / PUBLISH_FREQUENCY):
              # Sets the velocity to zero when this sinusoidal motion is completed
              sine_velocities = np.zeros(np.size(default_velocity))
            else:
              # Calculate sine wave positions
              sine_velocities = np.ones(np.size(default_velocity)) * amplitude * np.sin(2 * np.pi * SINE_FREQUENCY * elapsed_time)

            # Update joint positions
            for idx, joint_idx in enumerate(joint_indices):
                self.default_velocity[joint_idx] = sine_velocities[idx]

            # Update and publish joint state message if valid
            self.joint_command.header.stamp = self.get_clock().now().to_msg()
            self.joint_command.velocity = self.default_velocity.tolist()
    ...
    def execute_robot_motions(self):
        ...
        self.publish_sinusoidal_joint_velocity([f"left_arm_joint_{i}" for i in range(1, 8)], 0.2*np.pi*SINE_FREQUENCY)   # Unit: rad/sec

        self.get_logger().info("Move the right arm")
        self.publish_sinusoidal_joint_velocity([f"right_arm_joint_{i}" for i in range(1, 8)], 0.2*np.pi*SINE_FREQUENCY)  # Unit: rad/sec
